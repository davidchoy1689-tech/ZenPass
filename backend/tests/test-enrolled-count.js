...
function runTests() {
  ...
  assert(
    mismatches.cnt === 0,
    `所有 schedules 的 enrolled_count 與實際 booking 數量一致 (mismatches: ${mismatches.cnt})`,
  );
  ...
  assert(
    overflows.cnt === 0,
    `沒有 enrolled_count 超過 max_participants (overflows: ${overflows.cnt})`,
  );
  ...
  assert(
    negatives.cnt === 0,
    `沒有 enrolled_count 為負數 (negatives: ${negatives.cnt})`,
  );
  ...
  assert(
    undercounts.cnt === 0,
    `沒有 enrolled_count 低估 bookings (confirmed+attended) 數量 (undercounts: ${undercounts.cnt})`,
  );
