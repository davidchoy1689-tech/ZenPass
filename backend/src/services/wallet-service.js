 ...
 */
function creditCoachEarning({ coachId, scheduleId, coachEarningId, netAmount, description, bookingId }) {
  ...
  try {
    ...
    if (exists) {
      console.log(`[WALLET] Skipping duplicate credit for earning ${coachEarningId}`);
      return { success: true, transaction_id: exists.id, duplicate: true };
    }

    // Get current balance
    const user = db.prepare("SELECT wallet_balance FROM users WHERE id = ?").get(coachId);
    ...
    db.prepare("UPDATE users SET wallet_balance = ? WHERE id = ?").run(balanceAfter, coachId);
    ...
    `).run(
      txId, coachId, netAmount, balanceBefore, balanceAfter,
      bookingId ? 'booking' : 'schedule', bookingId || scheduleId,
      coachEarningId, description
    );

    console.log(`[WALLET] Credited HK$${netAmount} to coach ${coachId} (tx: ${txId})`);

    db.close();
    return { success: true, transaction_id: txId, balance_before: balanceBefore, balance_after: balanceAfter };
  } catch (err) {
    db.close();
 ...
 * @returns {object} { success, transaction_id, balance_after }
 */
function debitWallet({ userId, amount, type, description, reference, fee = 0, sourceType = '', sourceId = '' }) {
  const db = new Database(DB_PATH);
  db.pragma("foreign_keys = ON");
  ...
  try {
    ...
    if (balanceBefore < totalDebit) {
      db.close();
      return { success: false, error: `餘額不足 (需要 $${totalDebit}，現有 $${balanceBefore})` };
    }

    const balanceAfter = Math.round((balanceBefore - totalDebit) * 100) / 100;
    const txId = uuidv4();

    db.prepare("UPDATE users SET wallet_balance = ? WHERE id = ?").run(balanceAfter, userId);
    ...
    `).run(
      ...
      sourceType, sourceId, description, reference, fee
    );

    db.close();
    return { success: true, transaction_id: txId, balance_before: balanceBefore, balance_after: balanceAfter, fee };
  } catch (err) {
    db.close();
 ...
 * 攞完整交易記錄（可 filter）
 */
function getTransactions({ userId, type, limit = 50, offset = 0, startDate, endDate }) {
  const db = new Database(DB_PATH);
  db.pragma("foreign_keys = ON");
...
function getAllCoachWallets({ limit = 50, offset = 0 }) {
  ...
  try {
    ...
    const total = db.prepare("SELECT COUNT(*) as count FROM users WHERE is_coach = 1").get();
