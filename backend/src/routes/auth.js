...
router.post('/register', (req, res) => {
  try {
    ...
    const userRef = 'US-' + new Date().toISOString().slice(0,10).replace(/-/g,'') + '-' + Math.random().toString(36).substring(2,6).toUpperCase();
    ...
    const user = db.prepare('SELECT id, email, name, phone, credits, membership_type, created_at FROM users WHERE id = ?').get(id);
    db.close();
  ...
  try {
    ...
    if (user) {
      ...
      if (!user.auth_provider_id) {
        db.prepare('UPDATE users SET auth_provider = ?, auth_provider_id = ? WHERE id = ?')
    ...
    user = db.prepare('SELECT id, email, name, phone, credits, membership_type, created_at FROM users WHERE id = ?').get(id);
    db.close();
