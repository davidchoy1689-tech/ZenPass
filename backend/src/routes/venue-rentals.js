...
const { calcRentalCommission, getVenuePlan } = require("../services/commission");
...
router.get("/available", (req, res) => {
  try {
    ...
    if (district) { sql += ` AND v.district = ?`; params.push(district); }
    ...
    const total = db.prepare(`SELECT COUNT(*) as c FROM partner_venues WHERE status = 'active'`).get().c;
    db.close();
...
router.post("/book", authenticateToken, (req, res) => {
  try {
    if (req.user.role !== "coach") {
      return res.status(403).json({ success: false, error: "只限教練預約場地" });
    }
    
    const { venue_id, start_time, end_time, price_hkd } = req.body;
    if (!venue_id || !start_time || !end_time) {
      return res.status(400).json({ success: false, error: "請填寫場地、開始同結束時間" });
    }

    const db = new Database(DB_PATH);
    
    // Check venue exists and is active
    const venue = db.prepare("SELECT * FROM partner_venues WHERE id = ? AND status = 'active'").get(venue_id);
    if (!venue) {
      db.close();
      return res.status(404).json({ success: false, error: "場地不存在或未啟用" });
    }
...
router.delete("/:id/cancel", authenticateToken, (req, res) => {
  try {
    const db = new Database(DB_PATH);
    const rental = db.prepare("SELECT * FROM venue_rentals WHERE id = ?").get(req.params.id);
    if (!rental) {
      db.close();
    ...
    db.prepare("UPDATE venue_rentals SET status = 'cancelled' WHERE id = ?").run(req.params.id);
    db.close();
