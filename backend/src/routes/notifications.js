...
router.post('/push-subscribe', authenticateToken, (req, res) => {
  try {
    ...
    if (existing) {
      // 更新 user_agent
      db.prepare(`UPDATE push_subscriptions SET subscription = ?, user_agent = ?, updated_at = datetime('now') WHERE id = ?`)
        .run(JSON.stringify(subscription), req.headers['user-agent'] || null, existing.id);
    } else {
      db.prepare(`
        INSERT INTO push_subscriptions (id, user_id, subscription, user_agent)
        VALUES (?, ?, ?, ?)
      `).run(uuidv4(), req.user.id, JSON.stringify(subscription), req.headers['user-agent'] || null);
    }
