...
router.get('/credits', authenticateToken, (req, res) => {
  try {
    ...
    const user = db.prepare('SELECT credits, membership_type FROM users WHERE id = ?').get(req.user.id);
    ...
    res.json({ credits: user.credits, membership_type: user.membership_type, transactions });
