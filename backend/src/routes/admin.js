...
function requireAdmin(req, res, next) {
  ...
  const user = db.prepare('SELECT email, name, is_coach FROM users WHERE id = ?').get(req.user.id);
  db.close();

  const isAdmin = user && (
    user.email === 'david@zenpass.hk' ||
    user.email === 'admin@zenpass.hk' ||
    (user.name === 'David' || user.name === 'David Choy' || user.name === '管理員')
  );
  if (!isAdmin) {
...
router.post('/approve-payment', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ? AND status = ?').get(booking_id, 'pending_payment');
    if (!booking) {
      db.close();
