...
function _getUserVenue(userId) {
  ...
  try {
    ...
    return db
      .prepare(
        "SELECT * FROM partner_venues WHERE (email = ? OR user_id = ?) AND status = 'active'",
      )
  ...
  (req, res) => {
    try {
      ...
      if (status) {
        ...
        rows = db
          .prepare("SELECT * FROM partner_venues ORDER BY created_at DESC")
          .all();
      }
    ...
    try {
      ...
      return ok(res, {
        venue: {
          id: venue.id,
          name: venue.name,
          commission_plan: venue.commission_plan,
          commission_rate: venue.commission_rate,
        },
  ...
  (req, res) => {
    try {
      ...
      const computedCredits =
        credits_cost || Math.max(5, Math.round(price_hkd / 38));
...
router.put(
  ...
  (req, res) => {
    try {
      ...
      if (price_hkd !== undefined) {
        updates.push("price_hkd = ?");
        params.push(price_hkd);
      }
      ...
      if (credits_cost !== undefined) {
        updates.push("credits_cost = ?");
        params.push(credits_cost);
      }
  ...
  (req, res) => {
    try {
      ...
      const venues = venue_id
        ...
        : db
            .prepare("SELECT * FROM partner_venues WHERE status = 'active'")
            .all();
  ...
  (req, res) => {
    try {
      ...
      if (!role || !ROLE_HIERARCHY[role]) {
        return fail(
          res,
          `無效角色。可用角色: ${Object.keys(ROLE_HIERARCHY).join(", ")}`,
          400,
        );
      }
      ...
      db.prepare(
        "UPDATE users SET role = ?, partner_id = COALESCE(?, partner_id), updated_at = datetime('now') WHERE id = ?",
  ...
  (req, res) => {
    ...
    `,
      ...
      if (
        targetUser &&
        (!targetUser.role ||
          ROLE_HIERARCHY[targetUser.role] > ROLE_HIERARCHY.coach)
      ) {
        db.prepare(
          "UPDATE users SET role = ?, partner_id = ?, updated_at = datetime('now') WHERE id = ?",
        ).run(
          memberRole === "coach" ? "coach" : "partner_staff",
          venue.id,
          user_id,
        );
      }
...
router.put(
  ...
  (req, res) => {
    try {
      ...
      db.prepare(
        "UPDATE partner_venues SET status = ?, updated_at = datetime('now') WHERE id = ?",
      ).run(status, id);
...
router.put(
  ...
  (req, res) => {
    try {
      ...
      // Update venue owner
      db.prepare(
        "UPDATE partner_venues SET owner_id = ?, updated_at = datetime('now') WHERE id = ?",
      ).run(owner_id, id);

      // Update user role to partner_owner
      db.prepare(
        "UPDATE users SET role = 'partner_owner', partner_id = ?, updated_at = datetime('now') WHERE id = ?",
      ).run(id, owner_id);
