...
router.post("/join", authenticateToken, (req, res) => {
  try {
    ...
    if (existing) {
      db.close();
      return res.status(200).json({ message: "你已在候補名單中", waitlist_id: existing.id });
    }
...
function autoNotifyOnCancel(schedule_id) {
  try {
    ...
    console.log("🔔 Waitlist notified:", next.user_name, "for schedule", schedule_id);
  } catch (err) {
