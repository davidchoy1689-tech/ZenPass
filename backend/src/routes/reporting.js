...
router.get("/funnel", authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const usersAttended = db.prepare("SELECT COUNT(DISTINCT user_id) as c FROM bookings WHERE status='attended'").get().c;
    const repeatUsers = db.prepare("SELECT COUNT(DISTINCT user_id) as c FROM bookings GROUP BY user_id HAVING COUNT(*) > 1").length || 
      db.prepare("SELECT COUNT(*) as c FROM (SELECT user_id FROM bookings GROUP BY user_id HAVING COUNT(*) > 1)").get().c;
    const paidUsers = db.prepare("SELECT COUNT(DISTINCT user_id) as c FROM bookings WHERE payment_status='paid'").get().c;
    db.close();

    res.json({
      funnel: [
        { stage: "所有用戶", count: totalUsers },
        { stage: "已預約", count: usersWithBooking, rate: totalUsers > 0 ? Math.round(usersWithBooking/totalUsers*100) : 0 },
        { stage: "已付款", count: paidUsers, rate: totalUsers > 0 ? Math.round(paidUsers/totalUsers*100) : 0 },
        { stage: "已出席", count: usersAttended, rate: totalUsers > 0 ? Math.round(usersAttended/totalUsers*100) : 0 },
        { stage: "重複預約", count: repeatUsers, rate: totalUsers > 0 ? Math.round(repeatUsers/totalUsers*100) : 0 },
      ]
    });
  } catch (err) {
...
router.get("/retention", authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    // Overall retention
    const totalUsers = db.prepare("SELECT COUNT(*) as c FROM users").get().c;
    const active30d = db.prepare("SELECT COUNT(DISTINCT user_id) as c FROM bookings WHERE created_at > datetime('now', '-30 days')").get().c;
    const active90d = db.prepare("SELECT COUNT(DISTINCT user_id) as c FROM bookings WHERE created_at > datetime('now', '-90 days')").get().c;
    db.close();
    ...
    res.json({
      ...
      overall: {
        ...
        retention_30d: totalUsers > 0 ? Math.round(active30d/totalUsers*100) : 0,
        retention_90d: totalUsers > 0 ? Math.round(active90d/totalUsers*100) : 0,
      }
    });
  } catch (err) {
...
router.get("/revenue-trend", authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const totalRevenue = db.prepare("SELECT COALESCE(SUM(amount),0) as rev FROM bookings WHERE payment_status='paid'").get().rev;
    const pendingRevenue = db.prepare("SELECT COALESCE(SUM(amount),0) as rev FROM bookings WHERE payment_status='pending'").get().rev;
    const thisMonth = db.prepare("SELECT COALESCE(SUM(amount),0) as rev FROM bookings WHERE payment_status='paid' AND substr(created_at,1,7) = substr(datetime('now'),1,7)").get().rev;
    db.close();
