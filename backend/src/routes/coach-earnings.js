...
router.get('/earnings', authenticateToken, (req, res) => {
  try {
    ...
    // 用戶資訊 (commission)
    const user = db.prepare('SELECT commission_rate, total_earnings, pending_payout FROM users WHERE id = ?').get(req.user.id);
  ...
  try {
    ...
    const transaction = db.transaction(() => {
      for (const s of schedules) {
        ...
        const exists = db.prepare('SELECT id FROM coach_earnings WHERE schedule_id = ?').get(s.schedule_id);
        if (exists) continue;
        ...
        insertEarning.run(
          uuidv4(), s.coach_id, s.schedule_id, s.class_id,
          s.class_title, date, s.enrolled_count, s.price_hkd,
          gross, commissionRate, net
        );
        calculated++;
      }
    ...
    db.close();
    res.json({
      message: calculated > 0 ? `已自動計算 ${calculated} 筆收入` : '收入資料已是最新',
      calculated
    });
...
router.post('/payout-request', authenticateToken, (req, res) => {
  try {
    ...
    const { amount, payment_method, bank_name, bank_account, bank_code, fps_phone, payme_phone } = req.body;
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(payoutId, req.user.id, amount, fee, netPayout, payment_method || 'bank',
      bank_name || null, bank_account || null, bank_code || null,
      fps_phone || null, payme_phone || null);
    ...
    for (const e of earnings) {
      ...
      db.prepare('UPDATE coach_earnings SET status = ?, payout_id = ? WHERE id = ?')
    ...
    db.prepare('UPDATE users SET pending_payout = ? WHERE id = ?')
      .run(newPending.total, req.user.id);
...
router.get('/admin/all-earnings', authenticateToken, (req, res) => {
  try {
    // Admin check
    const user = new Database(DB_PATH).prepare('SELECT email FROM users WHERE id = ?').get(req.user.id);
    ...
    const total = db.prepare('SELECT COUNT(*) as count FROM coach_earnings').get();
    ...
    db.close();
    res.json({ list, summary, pagination: { page: parseInt(page), limit: parseInt(limit), total: total.count } });
...
router.post('/payout-process', authenticateToken, (req, res) => {
  try {
    ...
    const payout = db.prepare('SELECT * FROM coach_payouts WHERE id = ?').get(payout_id);
    ...
    if (action === 'approve') {
      db.prepare(`UPDATE coach_payouts SET status = 'processing', processed_by = ?, processed_at = datetime('now'), notes = ? WHERE id = ?`)
        .run(req.user.id, notes || null, payout_id);
    } else {
      // Reject — return earnings to pending
      db.prepare(`UPDATE coach_payouts SET status = 'rejected', processed_by = ?, processed_at = datetime('now'), notes = ? WHERE id = ?`)
        .run(req.user.id, notes || null, payout_id);
      db.prepare(`UPDATE coach_earnings SET status = 'pending', payout_id = NULL WHERE payout_id = ?`)
        .run(payout_id);
    }
    ...
    try {
      sendNotification('coach.payout_processed', {
        ...
        data: {
          ...
          reason: notes || (action === 'approve' ? '管理員已處理' : '提現申請未獲批'),
...
router.post('/private-income', authenticateToken, (req, res) => {
  try {
    const { date, description, amount, category, client_name, client_phone, notes } = req.body;
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, req.user.id, date, description, amount, category || '其他', client_name || null, client_phone || null, notes || null);
...
router.get('/private-income', authenticateToken, (req, res) => {
  try {
    ...
    const entries = db.prepare(`SELECT * FROM private_income ${where} ORDER BY date DESC, created_at DESC LIMIT ? OFFSET ?`).all(...params, parseInt(limit), offset);
    const total = db.prepare(`SELECT COUNT(*) as count FROM private_income ${where}`).get(...params);
    const totalAmount = db.prepare(`SELECT COALESCE(SUM(amount), 0) as total FROM private_income ${where}`).get(...params);
...
router.delete('/private-income/:id', authenticateToken, (req, res) => {
  try {
    const db = new Database(DB_PATH);
    const result = db.prepare('DELETE FROM private_income WHERE id = ? AND coach_id = ?').run(req.params.id, req.user.id);
    db.close();
    if (result.changes === 0) {
...
router.get('/settlements', authenticateToken, (req, res) => {
  try {
    ...
    let where = 'WHERE 1=1';
    const params = [];
    if (year) { where += ' AND period_year = ?'; params.push(parseInt(year)); }
    if (month) { where += ' AND period_month = ?'; params.push(parseInt(month)); }
    if (coach_id) { where += ' AND coach_id = ?'; params.push(coach_id); }
    if (status) { where += ' AND status = ?'; params.push(status); }
    
    const settlements = db.prepare(`SELECT s.*, u.name as coach_name, u.email as coach_email 
      FROM platform_settlements s
      LEFT JOIN users u ON s.coach_id = u.id
      ${where} ORDER BY s.period_year DESC, s.period_month DESC, s.created_at DESC`).all(...params);
    ...
    const summary = db.prepare(`SELECT 
      ...
      FROM platform_settlements ${where}`).get(...params);
    ...
    const transaction = db.transaction(() => {
      for (const e of earnings) {
        ...
        insertSettlement.run(
          uuidv4(), e.coach_id, e.coach_name, year, month,
          e.total_revenue || 0, commission, e.coach_payout || 0,
          e.class_count || 0, e.student_count || 0
        );
        count++;
      }
    });
...
router.post('/settlements/:id/pay', authenticateToken, (req, res) => {
  try {
    const db = new Database(DB_PATH);
    db.prepare(`UPDATE platform_settlements SET status = 'paid', paid_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`).run(req.params.id);
    db.close();
...
router.get('/settings/:key', (req, res) => {
  try {
    const db = new Database(DB_PATH);
    const setting = db.prepare('SELECT * FROM platform_settings WHERE key = ?').get(req.params.key);
    db.close();
...
router.put('/settings/:key', authenticateToken, (req, res) => {
  try {
    ...
    const db = new Database(DB_PATH);
    const existing = db.prepare('SELECT * FROM platform_settings WHERE key = ?').get(req.params.key);
    if (!existing) {
      db.prepare('INSERT INTO platform_settings (key, value, description) VALUES (?, ?, ?)').run(req.params.key, String(value), description || null);
    } else {
      db.prepare("UPDATE platform_settings SET value = ?, description = COALESCE(?, description), updated_at = datetime('now') WHERE key = ?")
        .run(String(value), description || null, req.params.key);
    }
    db.close();
    res.json({ message: '設定已更新', key: req.params.key, value: String(value) });
  } catch (err) {
    console.error(err);
...
router.get('/settings/effective-rate', (req, res) => {
  try {
    const db = new Database(DB_PATH);
    const rate = db.prepare("SELECT value FROM platform_settings WHERE key = 'coach_commission_rate'").get();
    db.close();
    res.json({ coach_rate: parseFloat(rate ? rate.value : 0.75), zenpass_rate: 1 - parseFloat(rate ? rate.value : 0.75) });
  } catch (err) {
