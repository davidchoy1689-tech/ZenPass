...
function migrate() {
  ...
  for (const u of adminUsers) {
    ...
    // David (super_admin)
    if (
      u.email === "david@zenpass.hk" ||
      u.email === "davidchoy1689@gmail.com"
    ) {
      db.prepare("UPDATE users SET role = 'super_admin' WHERE id = ?").run(
        u.id,
      );
    ...
    // Admin staff → platform_manager
      ...
      db.prepare("UPDATE users SET role = 'platform_manager' WHERE id = ?").run(
        u.id,
      );
    ...
    // Other zenpass.hk → platform_staff
      ...
      db.prepare("UPDATE users SET role = 'platform_staff' WHERE id = ?").run(
        u.id,
      );
  ...
  const coachUsers = db
    .prepare(
      "SELECT id, email FROM users WHERE is_coach = 1 AND (role IS NULL OR role NOT IN ('super_admin','platform_manager','platform_staff','admin'))",
    )
    .all();
  for (const u of coachUsers) {
    if (
      !u.role ||
      !ROLE_HIERARCHY[u.role] ||
      ROLE_HIERARCHY[u.role] > ROLE_HIERARCHY.coach
    ) {
  ...
  const noRoleUsers = db
    .prepare(
      "SELECT COUNT(*) as cnt FROM users WHERE role IS NULL OR role NOT IN ('super_admin','platform_manager','platform_staff','admin','partner_owner','partner_admin','partner_staff','coach','user','guest')",
    )
    .get();
  if (noRoleUsers.cnt > 0) {
    db.prepare(
      "UPDATE users SET role = 'user' WHERE role IS NULL OR role NOT IN ('super_admin','platform_manager','platform_staff','admin','partner_owner','partner_admin','partner_staff','coach','user','guest')",
    ).run();
    console.log(
      `[MIGRATE-RBAC] ✓ Mapped ${noRoleUsers.cnt} users with no/unknown role → 'user'`,
    );
  }
  ...
  if (!hasOwnerId) {
    ...
    console.log(
      "[MIGRATE-RBAC] ✓ owner_id column already exists on partner_venues",
    );
  }

  // Auto-link existing partner_venues to their user_id as owner
  const unlinkedVenues = db
    .prepare(
      "SELECT id, user_id FROM partner_venues WHERE owner_id IS NULL AND user_id IS NOT NULL",
    )
    .all();
  for (const v of unlinkedVenues) {
    db.prepare("UPDATE partner_venues SET owner_id = ? WHERE id = ?").run(
      v.user_id,
      v.id,
    );
  }
  console.log(
    `[MIGRATE-RBAC] ✓ Auto-linked ${unlinkedVenues.length} venues to owners`,
  );
  ...
  for (const v of unlinkedVenues) {
    ...
    if (
      ownerUser &&
      (!ownerUser.role ||
        ROLE_HIERARCHY[ownerUser.role] === undefined ||
        ROLE_HIERARCHY[ownerUser.role] > ROLE_HIERARCHY.partner_owner)
    ) {
      db.prepare(
        "UPDATE users SET role = 'partner_owner', partner_id = ? WHERE id = ?",
      ).run(v.id, v.user_id);
    }
  }
  ...
  if (!hasPartnerCol) {
    ...
    if (cols.some((c) => c.name === "partner_venue_id")) {
      ...
      `,
        ...
        .run();
      console.log(
        `[MIGRATE-RBAC] ✓ Synced ${linked.changes} classes partner_venue_id → partner_id`,
      );
    }
  } else {
  ...
  try {
    ...
    if (!cols.some((c) => c.name === "commission_rate")) {
      db.exec(
        "ALTER TABLE partner_members ADD COLUMN commission_rate REAL DEFAULT 0.75",
      );
    }
    if (!cols.some((c) => c.name === "updated_at")) {
      db.exec(
        "ALTER TABLE partner_members ADD COLUMN updated_at TEXT DEFAULT (datetime('now'))",
      );
    }
  } catch (e) {
    console.warn(
      "[MIGRATE-RBAC] ⚠️ partner_members column add failed:",
      e.message,
    );
  }

  // Auto-add existing coaches who taught at partner venues to partner_members
  const coachVenues = db
    .prepare(
      `
  ...
  console.log(
    `[MIGRATE-RBAC] ✓ Auto-linked ${coachVenues.length} coach-venue relationships`,
  );
