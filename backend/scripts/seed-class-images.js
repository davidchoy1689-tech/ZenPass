...
async function main() {
  ...
  // 取得所有分類
  const categories = db
    .prepare("SELECT DISTINCT category FROM classes ORDER BY category")
    .all();
  ...
  for (const { category } of categories) {
    ...
    const hasImage = db
      .prepare(
        "SELECT COUNT(*) as c FROM classes WHERE category = ? AND image_url IS NOT NULL AND image_url != ''",
      )
      .get(category);
    if (hasImage.c > 0) {
    ...
    try {
      ...
      const update = db.prepare(
        "UPDATE classes SET image_url = ? WHERE category = ? AND (image_url IS NULL OR image_url = '')",
      );
