...
const classes = db.prepare("SELECT id, title, category, duration FROM classes WHERE status='active'").all();
...
const existingFutureCount = db.prepare("SELECT COUNT(*) as c FROM class_schedules WHERE start_time >= datetime('now')").get().c;
...
const insertMany = db.transaction((schedules) => {
  for (const s of schedules) {
    insertStmt.run(s.id, s.class_id, s.start_time, s.end_time, s.max_participants, s.notes || '');
  }
});
...
for (const cls of classes) {
  ...
  else if (cat.includes('舞蹈') || cat.includes('dance')) timeOpts = TIME_SLOTS.dance;
  else if (cat.includes('健身') || cat.includes('fitness') || cat.includes('hiit')) timeOpts = TIME_SLOTS.fitness;
  else if (cat.includes('拳') || cat.includes('搏擊')) timeOpts = TIME_SLOTS.martial;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const venueNote = cls.title.includes('跑步') ? '海濱長廊' :
                      cls.title.includes('瑜伽') ? '瑜伽教室' :
                      cls.title.includes('HIIT') ? '健身室' :
                      cls.title.includes('木柱') || cls.title.includes('Mölkky') ? '戶外草地' : '';
