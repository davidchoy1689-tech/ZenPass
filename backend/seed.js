...
let coach = db.prepare("SELECT id FROM users WHERE email='coach@zenpass.hk'").get();
if (!coach) {
...
db.prepare("UPDATE users SET is_coach = 1 WHERE email='david@zenpass.hk'").run();
...
const transaction = db.transaction(() => {
  ...
  for (const cls of classes) {
    insertClass.run(
      String(cls.id), coach.id, cls.title, cls.desc || '',
      cls.category, cls.difficulty || 'beginner',
      cls.duration_min || 60, cls.capacity || 15, cls.price_hkd || 0,
      cls.location || '', cls.address || '', (cls.status === 'draft' ? 'inactive' : cls.status || 'active')
    );
    
    for (const sched of (cls.schedules || [])) {
      insertSchedule.run(
        sched.id, String(cls.id),
        sched.start_time, sched.end_time,
        sched.max_participants, 0, sched.status  // reset enrolled_count to 0, use actual bookings count
      );
    }
    count++;
  }
...
const schedCount = db.prepare('SELECT COUNT(*) as c FROM class_schedules').get();
