...
function openBookingModal(card) {
  ...
  // 從課程卡的 data-schedules 或全量數據獲取時段
  var schedules = [];
  var cid = card.getAttribute('data-id');
  if (cid && window.allCoursesData) {
    var found = window.allCoursesData.find(function(c) { return c.id === cid; });
...
async function confirmBooking() {
  ...
  try {
    ...
    var classId = currentCourse.courseId || currentCourse.id;
    // 攞 CSRF token
    ...
    var resp = await fetch((window.API_BASE || '/api') + '/bookings', {
      ...
      body: JSON.stringify({ schedule_id: datetime, class_id: classId, payment_type: 'credits', penalty_consent: true })
    });
    var data = await resp.json();
    if (!resp.ok || !data.success) {
      showToast('預約失敗', data.error || '請稍後再試', 'error');
    ...
    showToast('預約失敗', e.message || '請稍後再試', 'error');
