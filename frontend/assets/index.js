// ZenPass Index JS — extracted from index.html
      ...
      function renderClassCard(cls) {
        ...
        var imgStyle = cls.image_url
          ? 'background-image:url(' + cls.image_url + ');background-size:cover;background-position:center'
          : 'background:' + (categoryGradients[cls.category] || 'linear-gradient(135deg,#667eea,#764ba2)') + ';display:flex;align-items:center;justify-content:center;font-size:32px';
        var price = cls.price_hkd ? "HK$" + cls.price_hkd : "";
        var rating = cls.rating || "⭐4.8";
        var spots =
          cls.remaining_spots !== undefined ? cls.remaining_spots : null;
        var coach = cls.coach_name || "";
        var hot =
          cls.is_hot || cls.popular
            ? '<span class="badge-hot">🔥 熱門</span>'
            : "";
        var schedule =
        ...
        return (
          ...
          cls.id +
          "'\">" +
          '<div class="class-card-img" style="' + imgStyle + '">' +
          (cls.image_url ? '' : emoji) +
          "</div>" +
          '<div class="class-card-body">' +
