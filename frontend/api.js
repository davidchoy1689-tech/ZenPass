...
(function() {
    ...
    if (!isDavid && (name.indexOf('教練') > -1 || name.indexOf('導師') > -1 || name.indexOf('coach') > -1 || name.indexOf('Coach') > -1)) {
        role = 'coach';
    }
    ...
    if (!localStorage.getItem('zenpass_token')) {
        ...
        localStorage.setItem('zenpass_user', JSON.stringify({
            name: name, email: (isDavid ? 'david' : name.toLowerCase().replace(/\s/g,'')) + '@zenpass.hk',
            phone: '', role: role, credits: isDavid ? 999 : (role === 'coach' ? 0 : 45),
            bookings: 0, joined: new Date().toISOString().split('T')[0], avatar: isDavid ? '👤' : (role === 'coach' ? '🧘' : '🎓'),
...
function demoLogin(role) {
    const demoUser = {
        name: role === 'coach' ? '靜儀導師' : (role === 'admin' ? 'David Choy' : '張三同學'),
        email: role === 'coach' ? 'coach@zenpass.hk' : (role === 'admin' ? 'david@zenpass.hk' : 'student@zenpass.hk'),
        phone: role === 'coach' ? '9234 5678' : (role === 'admin' ? '9033 5538' : '9876 5432'),
        role: role,
    ...
    showToast('✅ 已登入為' + (role === 'coach' ? '教練 ' + demoUser.name : role === 'admin' ? '管理員 ' + demoUser.name : '學生 ' + demoUser.name), 'success');
...
var COURSES_JSON_URL = 'https://raw.githubusercontent.com/davidchoy1689-tech/ZenPass/main/courses.json';
...
function filterClasses(list, params) {
    ...
    if (params.search) {
        ...
        for (var i = 0; i < result.length; i++) {
            var cls = result[i];
            if ((cls.title && cls.title.toLowerCase().indexOf(q) > -1) ||
    ...
    if (params.sort === 'popular') {
        result = result.slice().sort(function(a, b) { return (b.popular ? 1 : 0) - (a.popular ? 1 : 0) || b.rating - a.rating; });
    }
    // Limit
    if (params.limit) {
...
const payments = {
    ...
    stripeIntent: (data) => apiRequest('POST', '/payments/stripe/create-intent', data),
...
function showLoginModal(callback) {
    ...
    tabs.forEach(tab => {
        tab.onclick = () => {
            ...
            document.getElementById('zen-login-form').classList.toggle('active', tab.dataset.tab === 'login');
            document.getElementById('zen-register-form').classList.toggle('active', tab.dataset.tab === 'register');
        };
    });
    ...
    document.getElementById('zen-login-form').onsubmit = async (e) => {
        ...
        try {
            ...
            if (result.user && !result.user.role) {
                result.user.role = result.user.email && result.user.email.includes('coach') ? 'coach' : 'student';
            }
    ...
    overlay.querySelectorAll('.zen-btn-social').forEach(btn => {
        btn.onclick = () => {
            showToast(`${btn.dataset.provider === 'apple' ? 'Apple' : 'Google'} 登入將在正式部署後啟用`, 'info');
        };
    });
}
...
function showUserMenu() {
    ...
    const close = (e) => { if (!menu.contains(e.target) && !e.target.closest('.zen-user-badge')) { menu.remove(); document.removeEventListener('click', close); } };
