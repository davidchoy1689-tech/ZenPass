...
test.describe("1️⃣ 首頁 / (Homepage)", () => {
  test("header, nav, categories, featured classes render correctly", async ({
    page,
  }) => {
    const errors = [];
    page.on("console", (msg) => {
    ...
    const catCount = await page
      .locator(
        '.category-chip, .cat-chip, [class*="category"] button, [class*="Category"] button',
      )
      .count();
    if (catCount === 0) {
    ...
    const featCount = await page
      .locator(
        "#featured-classes .class-card, #featured-classes .course-card, #featured-classes > *",
      )
      .count();
    ...
    expect(
      errors.filter((e) => !e.includes("favicon") && !e.includes("analytics")),
    ).toEqual([]);
  });
});
...
test.describe("2️⃣ Login /login.html", () => {
  test("login form renders and accepts credentials", async ({ page }) => {
    ...
    const navPromise = page
      .waitForURL("**/explore.html", { timeout: 8000 })
      .catch(() => null);
    await page.click("#login-btn");
    ...
    console.log(
      `[Login] Token: ${token ? "YES" : "NO"}, Navigated: ${!!navResult}`,
    );
...
test.describe("3️⃣ 探索課程 /explore.html", () => {
  test("class cards render, filter and search work", async ({ page }) => {
    ...
    const filterCount = await page
      .locator(
        '.category-chip, .filter-chip, [class*="filter"] button, [class*="Filter"] button',
      )
      .count();
    if (filterCount === 0) {
...
test.describe("4️⃣ 課程詳情 /class-detail.html", () => {
  test("schedules, price, waiver, booking button render", async ({ page }) => {
    ...
    await page.goto(`${BASE}/class-detail.html?id=${classId}`, {
    ...
    const schedCount = await page
      .locator(".schedule-item")
      .count()
      .catch(() => 0);
    console.log(`[ClassDetail] Schedules: ${schedCount}`);
    if (schedCount === 0)
      errors.push("Missing: schedule items (.schedule-item)");
    ...
    if (schedCount > 0) {
      ...
      console.log(
        `[ClassDetail] Waiver visible after schedule select: ${waiverVisible}`,
      );
      ...
      if (waiverVisible) {
        ...
        console.log(
          `[ClassDetail] Book button disabled after waiver tick: ${btnDisabled}`,
        );
      }
    }
...
test.describe("5️⃣ 會員方案 /membership.html", () => {
  test("plan cards display", async ({ page }) => {
    ...
    const planCount = await page
      .locator('.plan-card, [class*="plan"], [class*="Plan"]')
      .count();
    console.log(`[Membership] Plan cards: ${planCount}`);

    const cardCount = await page
      .locator("#plans-container > .card, #plans-container > div")
      .count();
...
test.describe("7️⃣ 我的預約 /my-bookings.html", () => {
  test("booking records display", async ({ page }) => {
    ...
    const bookingCount = await page
      .locator('.booking-card, [class*="booking"]')
      .count();
    console.log(`[MyBookings] Booking cards: ${bookingCount}`);

    const bodyText = await page.innerText("body").catch(() => "");
    if (
      !bodyText.includes("預約") &&
      !bodyText.includes("booking") &&
      !bodyText.includes("Booking") &&
      !bodyText.includes("課程")
    ) {
...
test.describe("9️⃣ 教練列表 /coaches.html", () => {
  test("coach cards display", async ({ page }) => {
    ...
    const coachCount = await page
      .locator('.coach-card, [class*="coach"]')
      .count();
    console.log(`[Coaches] Coach cards: ${coachCount}`);

    const bodyText = await page.innerText("body").catch(() => "");
    if (
      !bodyText.includes("教練") &&
...
test.describe("🔟 Admin /admin.html", () => {
  test("admin login and dashboard render", async ({ page }) => {
    ...
    if (await loginEmail.isVisible().catch(() => false)) {
      ...
      await page
        .locator(
          'button:has-text("登入"), button:has-text("Login"), button[type="submit"]',
        )
        .first()
        .click();
      await page.waitForTimeout(2000);
      ...
      const statsGrid = page.locator(
        '.stats-grid, .stat-card, [class*="stat"]',
      );
      ...
      console.log(
        "[Admin] Login form not found, may already be logged in or different layout",
      );
    }
...
test.describe("1️⃣1️⃣ API 驗證", () => {
  test("all critical API endpoints return valid responses", async ({
    request,
  }) => {
    const errors = [];
    ...
    console.log(
      `[API] Health raw: ${JSON.stringify(healthBody).substring(0, 200)}`,
    );
    // Health API wraps response in {success, data}
    const healthData = healthBody?.data || healthBody;
    const dbConnected =
      healthData?.database?.connected || healthData?.status === "ok";
    ...
    const classCount = Array.isArray(classesBody)
      ? classesBody.length
      : classesBody?.data?.length || 0;
