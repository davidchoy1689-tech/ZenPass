...
(function () {
  ...
  Z.modal = function (opts) {
    ...
    modal.innerHTML = [
      ...
      opts.desc
        ...
        : "",
      opts.confirmText
        ? '<button class="zp-modal-btn primary">' +
          Z.escHtml(opts.confirmText) +
          "</button>"
        : "",
      opts.cancelText
        ? '<button class="zp-modal-btn secondary" style="margin-top:6px">' +
          Z.escHtml(opts.cancelText) +
          "</button>"
        : "",
    ].join("");
  ...
  Z.showLoading = function (text) {
    ...
    el.innerHTML =
      '<div style="text-align:center"><div style="display:inline-block;width:36px;height:36px;border:3px solid #e5e7eb;border-top:3px solid #ff6b35;border-radius:50%;animation:zpSpin 0.8s linear infinite;margin-bottom:8px"></div><div style="font-size:13px;color:#6b7280">' +
      Z.escHtml(text) +
      "</div></div>";
  ...
  Z.init = function () {
    ...
    if (btns && Z.isLoggedIn()) {
      var user = Z.getUser();
      btns.innerHTML =
        '<a href="my.html" class="zen-btn-ghost" style="font-size:13px">👤 ' +
        (user.name || "我的") +
        "</a>";
    }
  };
